...
bool LinkedList<T>::remove(T myData) {
    //Gets index of data to be deleted
    Node<T>* ptr = head;
    for (int i = 0; i < indexOf(myData); i++)
        ptr = ptr->next;
    //Logic if node to be deleted is first
    if (head == ptr) {
        if (head->next == NULL) {
            cout << "There is only one node, cannot remove all nodes." << endl;
            return false;
        }
        ...
        //Otherwise do normal deleting process
        Node<T>* previous = head;
        while (previous->next != NULL && previous->next != ptr) {
...
int LinkedList<T>::lastIndexOf(T myData) const {
    //Modified indexOf to not return but count up position
    Node<T>* ptr = head;
    int position = -1; //if the value is never found this is what will be returned
    for (int i = 0; i < size; i++) {
        if (ptr->data == myData)        
            position = i;
        ptr = ptr->next;
...
bool LinkedList<T>::set(int index, T myData) {
    ...
    ////Logic if node to be cahnged is first
    //if (head == ptr) {
    //    temp = head->data;
    //    head->data = myData;
    //    return temp;
